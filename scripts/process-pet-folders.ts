...
import { parsePetMetadata } from './lib/metadata-parser';
...
for (const blogId of blogIds) {
  ...
  for (const keyword of keywords) {
    ...
    const gongFiles = files.filter((f) => f.includes('공정위문구') && isImageFile(f));
    gongFiles.forEach((f) => {
    ...
    const restFiles = files
      .filter((f) => !f.includes('공정위문구') && isImageFile(f))
    ...
    if (fs.existsSync(txtPath)) {
      ...
      const metadata = parsePetMetadata(content);
